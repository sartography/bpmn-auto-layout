export class Grid {
  ...
  adjustColumnForMultipleIncomingToMedian(elements, currentElement) {
    const results = elements.map(element => this.find(element));
    const cols = results
      .map(result => result[1])
      .filter(col => col >= 0);
    ...
    const targetCol = Math.round(cols.reduce((sum, col) => sum + col, 0) / cols.length);
    const [ row , col ] = this.find(currentElement);

    if (targetCol !== col && !this.grid[row][targetCol]) {
      this.grid[row][targetCol] = currentElement;
      this.grid[row][col] = null;
    }
  }
