import { DEFAULT_TASK_HEIGHT, getDefaultSize, is } from '../di/DiUtil.js';
import { isSharedGatewayRejectionSink } from './flowNodeUtil.js';

// A 100 px task must retain at least 65 px of clear connector run before the
// next task.
export const DEFAULT_CELL_WIDTH = 165;
 ...
 * @returns waypoints
 */
export function connectElements(source, target, layoutGrid, connection) {
  ...
  if (dX < 0) {
    ...
    if (dY > 0 && isSharedGatewayRejectionSink(target)) {
      return [
  ...
  if (dY === 0) {
    if (isDirectPathBlocked(source, target, layoutGrid)) {
      ...
      const gatewayChainFlows = (source.outgoing || [])
        .filter(flow => is(flow.targetRef, 'bpmn:ExclusiveGateway'));
      if (is(source, 'bpmn:ExclusiveGateway')
        && is(target, 'bpmn:ExclusiveGateway')
        && gatewayChainFlows[0] === connection) {
        const routeY = sourceBounds.y - DEFAULT_CELL_HEIGHT / 2;
...
export function getBounds(element, row, col, shift, attachedTo) {
  ...
  if (!attachedTo) {
    return {
      ...
      x: (col * DEFAULT_CELL_WIDTH) + (DEFAULT_CELL_WIDTH - width) / 2 + x + (element.layoutOffset?.x || 0),
      y: row * DEFAULT_CELL_HEIGHT + (DEFAULT_CELL_HEIGHT - height) / 2 + y + (element.layoutOffset?.y || 0)
    };
  }
