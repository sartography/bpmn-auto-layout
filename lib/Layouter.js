  ...
  setExpandedPropertyToModdleElements(bpmnModel) {
    ...
    if (allElements) {
      for (const element of Object.values(allElements)) {
        ...
        // A deliberately empty subprocess plane is a request to generate the
        // drill-down DI (used by transient renderers and candidate builders).
        if (element.$type === 'bpmndi:BPMNPlane' && is(element.bpmnElement, 'bpmn:SubProcess')) {
