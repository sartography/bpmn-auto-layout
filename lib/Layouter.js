    ...
    while (visited.size < elements.filter(element => !element.attachedToRef).length) {
      ...
      elementsToAdd.forEach((el, index) => {
        if (isDisconnectedFlowNode(root, el)) {
          addToGridStartOrAfterExistingGrid(grid, el, index);
        } else {
          grid.add(el);
        }
        visited.add(el);
      ...
      if (grid.getElementsTotal() !== elements.length) {
        ...
        if (missingElements.length > 0) {
          ...
          if (isDisconnectedFlowNode(root, missingElement)) {
            addToGridStartOrAfterExistingGrid(grid, missingElement, 0);
          } else {
...
function addToGridStartOrAfterExistingGrid(grid, element, rowOffset) {
  ...
  grid.add(element, [ rowOffset % rowCount, colCount ]);
}
