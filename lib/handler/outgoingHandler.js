...
import {
  ...
  SHARED_GATEWAY_REJECTION_END_Y_OFFSET
...
export default {
  'addToGrid': ({ element, grid, visited, stack }) => {
    ...
    outgoing.forEach((nextElement, index, arr) => {
      ...
      if (!previousElement && shouldPlaceOutgoingBelow(element, nextElement, outgoing)) {
        nextElement.layoutOffset = {
          ...(nextElement.layoutOffset || {}),
          y: SHARED_GATEWAY_REJECTION_END_Y_OFFSET
        };
      ...
      else if (shouldPlaceGatewayEndEventBelowGateway(element, nextElement)) {
        grid.addBelow(element, nextElement);
      }
  ...
  'createConnectionDi': ({ element, row, col, layoutGrid, diFactory }) => {
    ...
    return outgoing.map(out => {
      ...
      const waypoints = connectElements(element, target, layoutGrid, out);
...
function getOutgoing(element) {
  // Preserve semantic/XML order. A default flow is an execution fallback,
  // not a statement that the branch deserves visual priority.
  return element.outgoing || [];
}

function shouldPlaceOutgoingBelow(element, nextElement, outgoing) {
  return outgoing.length === 1
    && isSharedGatewayRejectionSink(element)
    && is(nextElement, 'bpmn:EndEvent');
}

function shouldPlaceGatewayEndEventBelowGateway(element, nextElement) {
